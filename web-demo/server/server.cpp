...
private:
	void on_read(const error_code & err, size_t bytes) {
		...
		if (username_=="NULL")
			{
			username_ = login(m,id_);//登陆处理
			do_write(login_ok(id_));//登陆成功
			}
		else
			{
			merge(m,id_);//合并信息块
			do_write(info(id_));//当前信息
			}
		}

	void on_write(const error_code & err, size_t bytes)
		{
		if(alive(id_))do_read();//是否继续连接
		else stop();
		return;
		}
...
int now_opened;
talk_to_client::ptr clients[max_connect];
...
void handle_accept(talk_to_client::ptr client, const talk_to_client::error_code & err)
	...
	if(now_opened<max_connect)acceptor.async_accept(new_client->sock(), boost::bind(handle_accept, new_client, _1));
	++now_opened;
	}
